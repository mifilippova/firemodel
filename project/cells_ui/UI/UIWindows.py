...
class UIWindows:
    ...
    def open_project(self):
        ...
        file_name, _ = QFileDialog.getOpenFileName(self.main_window.element, "Open File", "",
                                                   "FireMap (*.fmap)", options=options)
        if file_name:
            self.main_window.elements["mainContent"].removeWidget(self.web)
            self.web.deleteLater()

            self.web = QWebEngineView()
            self.loading_view = MapView(self.web, save_file_path=file_name, ui=self)

    def project_opened(self, status, web_engine, error_message=None):
        if status:
            self.web = web_engine
            self.main_window.elements["mainContent"].addWidget(web_engine, stretch=1)
            self.map_view = self.loading_view
            self.loading_view = None
            self.update_layers_list()
        else:
    ...
    def save_project(self):
        if self.map_view.save_file_path is not None:
            self.map_view.save()
        else:
    ...
    def save_project_as(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(self.main_window.element, "Save File", "",
                                                   "FireMap (*.fmap)", options=options)
        if file_name:
            self.map_view.save_file_path = file_name
            self.save_project()
